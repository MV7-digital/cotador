...
;(function($){
    ...
    function atualizarTotal(){
        ...
        for(let i = 0; i < planos; i++){
            for(let j = 0; j < 10; j++){
                vidas = $(`#idade_${i}_${j}`).val();
                totalPreco[i] += parseFloat($(`#unidade_${i}_${j}`).html()) * vidas;
                totalPrecoC[i] += parseFloat($(`#unidadec_${i}_${j}`).html()) * vidas;
            ...
            $(`#pdf_total_${i}`).text(totalPreco[i].toFixed(2).replace('.', ','));
            $(`#pdf_totalc_${i}`).text(totalPrecoC[i].toFixed(2).replace('.', ','));
    ...
    $('#previsualizar2').click(function() {
        ...
        for(let i = 0; i < 6; i++){
           for(let j = 0 ; j < 10; j++){
               $(`#pdf_valor_${i}_${j}`).html($(`#unidade_${i}_${j}`).text());
               $(`#pdf_valorc_${i}_${j}`).html($(`#unidadec_${i}_${j}`).text());
